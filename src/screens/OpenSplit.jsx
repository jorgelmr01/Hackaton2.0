...
const OpenSplit = ({ setShowBottomNav }) => {
  ...
  const [step, setStep] = useState('select') // select, configure, confirm

  useEffect(() => {
    setShowBottomNav(true)
  }, [setShowBottomNav])

  // Pre-select transactions if coming from Tarjetas
  useEffect(() => {
  ...
  const transactions = [
    ...
    { id: 4, name: 'Compra En Línea En Dlo Didi Rides', amount: 200.00, date: 'oct 20 2025', location: 'Mexico, Dmm' },
  ]
  ...
  const [splitType, setSplitType] = useState('equal') // equal, proportional
  const [showAddExternal, setShowAddExternal] = useState(false)
  const [externalName, setExternalName] = useState('')
  const [externalPhone, setExternalPhone] = useState('')
  const [amounts, setAmounts] = useState({}) // per friend id amount when proportional
  ...
  const getAmountPerPerson = () => {
    const selectedFriendsCount = friends.filter(f => f.selected).length + 1 // +1 for user
  ...
  const proportionalSum = () => {
    // include user amount if provided under key 'me'
    const friendSum = friends.filter(f => f.selected).reduce((sum, f) => sum + (amounts[f.id] || 0), 0)
  ...
  const canContinueFromConfigure = () => {
    ...
    // proportional: sum must equal total within a small epsilon
          ...
          <div className="configure-split">
            ...
            <div className="friends-selection">
              <div className="section-title">
                Seleccionar amigos
                <button className="text-button" onClick={() => setShowAddExternal(!showAddExternal)}>+ Agregar externo</button>
              </div>
              {showAddExternal && (
                <div className="add-external-panel">
                  ...
                  <div className="add-external-actions">
                    <button className="secondary-button" onClick={() => { setShowAddExternal(false); setExternalName(''); setExternalPhone('') }}>Cancelar</button>
                    <button
                      className="primary-button"
                      onClick={() => {
                        if (!externalName.trim() || !externalPhone.trim()) return
                        const id = Date.now()
                        setFriends(prev => [...prev, { id, name: externalName.trim(), initials: externalName.split(' ').map(n => n[0]).join('').toUpperCase().slice(0,2), selected: true, externo: true, phone: externalPhone.trim() }])
